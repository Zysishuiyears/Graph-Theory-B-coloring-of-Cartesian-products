...
# ============================================================
# ====== 批量检验：C_odd x P2^m 与 Δ 的关系 =====================
# ============================================================

def sweep_odd_cycle_with_p2(
    odd_cycle: int,
    max_p2: int,
    k_min: Optional[int] = None,
    k_max: Optional[int] = None,
    dump_one_solution: bool = False,
) -> str:
    """
    固定一个奇圈 C_odd，枚举 C_odd x P2^m (m=1..max_p2)。
    对每个 m，从 k_min 到 k_max 依次判定 SAT，
    记录最小可行 k（若都 UNSAT 则记为 >k_max）。
    """
    if odd_cycle < 3 or odd_cycle % 2 == 0:
        raise ValueError("odd_cycle must be an odd integer >= 3.")
    if max_p2 < 1:
        raise ValueError("max_p2 must be >= 1.")
    if k_min is not None and k_min < 1:
        # treat non-positive as "auto"
        k_min = None
    ...
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = os.path.join(RESULTS_ROOT, f"{stamp}_C{odd_cycle}_P2^{max_p2}_SWEEP")
    os.makedirs(out_dir, exist_ok=True)

    summary_path = os.path.join(out_dir, "summary.csv")
    with open(summary_path, "w", encoding="utf-8") as fsum:
        fsum.write("factors,m_p2,delta,check_from_k,check_upto_k,first_sat_k,relation\n")

        for m in range(1, max_p2 + 1):
            cycles = [odd_cycle]
            paths = [2] * m
            graph = ProductGraph(cycles + paths, [True] + [False] * m)
            delta = max(len(graph.incident[v]) for v in graph.vertices) if graph.vertices else 0
            k_start = k_min if k_min is not None else delta
            k_end = k_max if k_max is not None else (delta + 1)

            first_sat_k = None
            for k in range(k_start, k_end + 1):
                sat, _, _ = decide_existence_B(cycles=cycles, paths=paths, k=k, dump_one_solution=dump_one_solution)
                if sat:
                    first_sat_k = k
                    break

            if first_sat_k is None:
                relation = f">k_max={k_end}"
                first_sat_str = ""
            else:
                diff = first_sat_k - delta
                if diff == 0:
                    relation = "Δ"
                elif diff > 0:
                    relation = f"Δ+{diff}"
                    ...
                    relation = f"Δ{diff}"
                first_sat_str = str(first_sat_k)

            factor_str = "x".join([f"C{odd_cycle}"] + ["P2"] * m)
            fsum.write(f"{factor_str},{m},{delta},{k_start},{k_end},{first_sat_str},{relation}\n")

    print("Sweep results saved to:", summary_path)
    return summary_path
...
if __name__ == "__main__":
    ...
    # C3 □ P7
    # decide_existence_B(cycles=[5], paths=[2,2,2], k=5)
    # sweep_odd_cycle_with_p2(odd_cycle=3, max_p2=6, k_extra=1, dump_one_solution=False)
    sweep_odd_cycle_with_p2(odd_cycle=5, max_p2=6, k_min=0, k_max=3, dump_one_solution=False)
