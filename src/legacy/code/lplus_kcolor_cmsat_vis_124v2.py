...
# 瀹為獙鍚嶅瓧锛岀敤鏉ュ尯鍒嗕笉鍚屽疄楠?
EXPERIMENT_NAME = "C3xC3_k5"

# 鏄惁鎶?CNF 鏂囦欢淇濆瓨鍒?results/runs/legacy_2026/_early_outputs/ 鐩綍锛圱rue/False锛?
WRITE_CNF_FILE = True
CNF_FILENAME = "C3xC3_k5.cnf"   # 鍙湪 WRITE_CNF_FILE=True 鏃剁敤
...
def plot_lplus_graph(adj_Lplus, colors, out_path, figsize=(8, 8)):
    ...
    - 鑺傜偣缂栧彿涓?0..|E|-1
    - 鑻ユ彁渚涗簡 colors锛屽垯鐢?colormap 鎸夐鑹叉秱鐐癸紝
      骞跺湪鍙充笂瑙掔粰鍑虹鏁ｅ浘渚嬶細color 1, color 2, ...
    """
    n = len(adj_Lplus)
    G = nx.Graph()
    ...
    if colors is not None and len(colors) == n:
        max_color_idx = max(colors) if colors else 0
        # 绂绘暎 colormap锛?..max_color_idx
        cmap = plt.get_cmap("tab10", max_color_idx + 1)

        # 鑺傜偣棰滆壊鏁扮粍锛堣浆鎹㈡垚 RGBA锛?
        node_rgba = [cmap(c) for c in colors]
        ...
        for c in used_colors:
            # label 鐢?1-based锛坈olor 1, 2, ...锛夛紝浣犺 0-based 灏辨妸 +1 鍘绘帀
            label = f"color {c + 1}"
            handle = plt.Line2D(
                [], [],              # 涓嶉渶瑕佸疄闄呮暟鎹?
                color=cmap(c),
