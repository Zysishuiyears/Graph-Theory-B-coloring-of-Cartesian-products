...
# === 新增：汇总“不同副本之间的连接线”（沿 slice_axis 的边）着色情况 ===
def report_intercopy_colors(dims, edges, colors, slice_axis=2, export_csv=True, prefix="intercopy"):
    """
    对于 d>=2 的 C_{n1} x ... x C_{nd}：
      固定 slice_axis，统计所有 (固定其它坐标) 在该轴上的“向下一层”的边：
        (i1,...,is,...,id) --(沿 slice_axis)--> (i1,...,is+1 mod ns,...,id)
      并报告其颜色。
    - 若 d==3，会按剩余两轴排成矩阵打印（更直观）。
    - 同时导出 CSV：每行是一条跨副本边（含端点坐标、eid、color）。
    """
    ...
    # 一个便捷的查询：给定两个顶点，取该边颜色
    def edge_color(u, v):
        key = (u, v) if u < v else (v, u)
        eid = e2id[key]
        return colors[eid], eid

    # 收集所有“沿 slice_axis”的跨层边
    rows = []
    remain_axes = [ax for ax in range(d) if ax != slice_axis]
    ns = dims[slice_axis]
    ...
    for v in range(Vn):
        ...
        # 这条边是否就是“沿 slice_axis”的那条？（按我们的构图，确实是）
    ...
    # === 打印易读版（d==3 时排成矩阵；其他维数打印计数） ===
    if d == 3:
        a, b = remain_axes   # 矩阵坐标两轴
        na, nb = dims[a], dims[b]
        for s in range(ns):
            # 构造 na x nb 矩阵：位置 (ia, ib) 对应坐标
...
def main():
    ...
    #     plot_lower_dim_slices_curvy(dims, info["edges"], colors, Kc,
    #                             slice_axis=(len(dims)-1))  # 或 0/1/2 指定维
    if PLOT_SLICES_LOWER:
