...
CMSAT_THREADS = min(12, os.cpu_count() or 1)  # i5-13500H: 12C/16T，先用 12
CMSAT_VERB = 0  # 降低输出，减少 I/O

# 约束开关：是否要求每个 4-cycle “同色最多一次”（你的旧代码里的 C4 约束）
USE_C4_DISTINCT = True

# 搜索停止条件（搜索型停止）
MAX_SOLVER_CALLS = 10000          # 最多调用 solver 的次数（上限保护）
MAX_NO_NEW_CLASS = 500            # 连续这么多次没有出现“新等价类”就停
MAX_NO_BEST_IMPROVE = 2000        # 连续这么多次 best 分数不提升就停
...
def run_cmsat(cnf_path: str) -> Optional[List[int]]:
    cmd = [
        CMSAT_PATH,
        "-t", str(CMSAT_THREADS),   # 或 "--threads", str(...)
        "--verb", str(CMSAT_VERB),
        cnf_path
    ]
...
def draw_graph_edge_coloring(H, V, m, n, k, path):
    ...
    for i in range(m):
        for j in range(n):
            ...
            v = (i, (j + 1) % n)
            c = H[i][j]
            G.add_edge(u, v, color=c, label=str(c + 1))
    ...
    # 用同一个 edgelist 来画边、取颜色、取标签（保证一一对应）
    edgelist = list(G.edges())
    edge_colors = [cmap(G[u][v]["color"] % 10) for (u, v) in edgelist]

    # 无向边的稳定 key：frozenset({u,v})
    label_by_ue = {frozenset((u, v)): G[u][v]["label"] for (u, v) in edgelist}

    # 画图
    plt.figure(figsize=(10, 6))
    nx.draw_networkx_edges(G, pos, edgelist=edgelist, edge_color=edge_colors, width=2.2)
    ...
    # 顶点标号
    node_labels = {(i, j): f"({i},{j})" for i in range(m) for j in range(n)}
    nx.draw_networkx_labels(G, pos, labels=node_labels, font_size=8)

    # 边标签：用我们自己的“无向 key”匹配，避免 (u,v)/(v,u) 问题
    # draw_networkx_edge_labels 仍然要求键是 (u,v)，所以我们重建一次 dict：
    edge_label_dict = {(u, v): label_by_ue[frozenset((u, v))] for (u, v) in edgelist}
